...
#define app_chk(cond, desc)                            \
  do {                                                 \
    if (!(cond)) {                                     \
      log_error("[app] app_chk %s: %s", (cond), desc); \
      if (APP_CHK_FATAL)                               \
        exit(1);                                       \
    }                                                  \
  } while (0)
...
#define app_vkchk(cond, desc)                                \
  do {                                                       \
    if ((cond) != VK_SUCCESS) {                              \
      log_error("app_vkchk %d: %s", (uint32_t)(cond), desc); \
      if (APP_CHK_FATAL)                                     \
        exit(1);                                             \
    }                                                        \
  } while (0)
...
#define app_vkchk_sc(cond, update_flag)                                         \
  do {                                                                          \
    ...
    if (res < VK_SUCCESS) {                                                     \
      ...
      log_error("app_vkchk_sc: %d", (int)res);                                  \
      if (APP_CHK_FATAL)                                                        \
        exit(1);                                                                \
    }                                                                           \
  } while (0)

#define APP_COLOR_SRGBA(r, g, b, a) pow((r), 2.2f), pow((g), 2.2f), pow((b), 2.2f), a
